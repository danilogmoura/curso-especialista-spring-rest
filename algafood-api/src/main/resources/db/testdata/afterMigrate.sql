...
select setval('estado_id_seq', (select max(id) from estado), true);
...
select setval('cidade_id_seq', (select max(id) from cidade), true);
...
select setval('cozinha_id_seq', (select max(id) from cozinha), true);


insert into restaurante (id, nome, taxa_frete, cozinha_id, data_cadastro, data_atualizacao, endereco_cidade_id, endereco_cep, endereco_logradouro, endereco_numero, endereco_bairro) values (1, 'Thai Gourmet', 10, 1, now() at time zone 'utc', now() at time zone 'utc', 1, '38400-999', 'Rua João Pinheiro', '1000', 'Centro');
insert into restaurante (id, nome, taxa_frete, cozinha_id, data_cadastro, data_atualizacao) values (2, 'Thai Delivery', 9.50, 1, now() at time zone 'utc', now() at time zone 'utc');
insert into restaurante (id, nome, taxa_frete, cozinha_id, data_cadastro, data_atualizacao) values (3, 'Tuk Tuk Comida Indiana', 15, 2, now() at time zone 'utc', now() at time zone 'utc');
insert into restaurante (id, nome, taxa_frete, cozinha_id, data_cadastro, data_atualizacao) values (4, 'Java Steakhouse', 12, 3, now() at time zone 'utc', now() at time zone 'utc');
insert into restaurante (id, nome, taxa_frete, cozinha_id, data_cadastro, data_atualizacao) values (5, 'Lanchonete do Tio Sam', 11, 4, now() at time zone 'utc', now() at time zone 'utc');
insert into restaurante (id, nome, taxa_frete, cozinha_id, data_cadastro, data_atualizacao) values (6, 'Bar da Maria', 6, 4, now() at time zone 'utc', now() at time zone 'utc');

select setval('restaurante_id_seq', (select max(id) from restaurante), true);
...
select setval('forma_pagamento_id_seq', (select max(id) from forma_pagamento), true);


insert into permissao (id, nome, descricao) values (1, 'CONSULTAR_COZINHAS', 'Permite consultar cozinhas');
insert into permissao (id, nome, descricao) values (2, 'EDITAR_COZINHAS', 'Permite editar cozinhas');

select setval('permissao_id_seq', (select max(id) from permissao), true);
